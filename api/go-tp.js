...
export default async (req, res) => {
  try {
    const url = new URL(req.url, 'http://x'); // compat per vercel
    const target = url.searchParams.get('target'); // URL Aviasales/Hotellook "pulita"
    const subid  = cleanRef(url.searchParams.get('subid') || '');
    if (!target) return res.status(400).send('Missing target');

    // Costruisci link affiliato via Links API (marker + sub_id)
    const body = {
