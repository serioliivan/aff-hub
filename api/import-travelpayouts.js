...
async function fetchTp(programId, sinceDate) {
  const body = {
  ...
  if (!r.ok) throw new Error('TP http ' + r.status);
...
export default async (req, res) => {
  try {
    ...
    let rows = [];
    for (const pid of programs) rows = rows.concat(await fetchTp(pid, sinceDate));
